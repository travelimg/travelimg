...
public class GeoServiceImpl implements GeoService{
    ...
    public Place getPlaceByGeoData(double latitude, double longitude) throws ServiceException {
        String json = readUrl("http://maps.googleapis.com/maps/api/geocode/json?latlng=" + latitude + "," + longitude);
        ...
        for(int i = 0; i<addressComponentsArray.length(); i++){
            if(addressComponentsArray.getJSONObject(i).getJSONArray("types").get(0).equals("locality")){
                p.setCity(addressComponentsArray.getJSONObject(i).getString("long_name"));
            }
            if(addressComponentsArray.getJSONObject(i).getJSONArray("types").get(0).equals("country")){
