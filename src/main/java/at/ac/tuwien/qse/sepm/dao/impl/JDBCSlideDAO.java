...
public class JDBCSlideDAO extends JDBCDAOBase implements SlideDAO {
    ...
    private static final String TITLE_READ_ALL_BY_SLIDESHOW_STATEMENT =
            "SELECT Slide.id, slideshow_id, orderposition, caption, color FROM TitleSlide JOIN Slide ON TitleSlide.id = Slide.id WHERE slideshow_id=? ORDER BY orderposition asc;";
    ...
    private static final String PHOTO_READ_ALL_BY_SLIDESHOW_STATEMENT =
            "SELECT Slide.id, slideshow_id, orderposition, caption, photo_id FROM PhotoSlide JOIN Slide ON PhotoSlide.id = Slide.id WHERE slideshow_id=? ORDER BY orderposition asc;";
    ...
    public PhotoSlide update(PhotoSlide slide) throws DAOException, ValidationException {
        ...
        try {
            jdbcTemplate.update(PHOTO_UPDATE_STATEMENT,
    ...
    public TitleSlide update(TitleSlide slide) throws DAOException, ValidationException {
        ...
        try {
            jdbcTemplate.update(TITLE_UPDATE_STATEMENT,
                    slide.getColor(),
                    slide.getId()
            );
    ...
    @Override
    public List<PhotoSlide> getPhotoSlidesForSlideshow(int slideshowId) throws DAOException {
        try {
            return jdbcTemplate.query(PHOTO_READ_ALL_BY_SLIDESHOW_STATEMENT, new PhotoSlideMapper(), slideshowId);
    ...
    @Override
    public List<MapSlide> getMapSlidesForSlideshow(int slideshowId) throws DAOException {
        try {
            return jdbcTemplate.query(MAP_READ_ALL_BY_SLIDESHOW_STATEMENT, new MapSlideMapper(), slideshowId);
        } catch (DataAccessException | DAOException.Unchecked ex) {
    ...
    @Override
    public List<TitleSlide> getTitleSlidesForSlideshow(int slideshowId) throws DAOException {
        try {
            return jdbcTemplate.query(TITLE_READ_ALL_BY_SLIDESHOW_STATEMENT, new TitleSlideMapper(), slideshowId);
        } catch (DataAccessException | DAOException.Unchecked ex) {
    ...
    private Slide updateBase(Slide slide) throws DAOException, ValidationException {
        ...
        try {
            jdbcTemplate.update(BASE_UPDATE_STATEMENT,
